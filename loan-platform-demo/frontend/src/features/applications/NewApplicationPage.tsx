  ...
  return <div className="page form-page">
    ...
    <div className="form-layout">
      <section className="panel form-card">
        ...
        <h1>Základní údaje o úvěru</h1><p className="lead">Založte žádost pro firemního klienta. Po uložení ji můžete předat k posouzení.</p>
        ...
        <form onSubmit={event => void form.handleSubmit(values => mutation.mutate(values))(event)} noValidate>
          <label className="field"><span>ID firemního klienta <i>*</i></span><input {...form.register('customerId')} placeholder="např. CORP-12345" aria-invalid={!!form.formState.errors.customerId} />{form.formState.errors.customerId && <small>{form.formState.errors.customerId.message}</small>}</label>
          <div className="field-row"><label className="field amount-field"><span>Požadovaná částka <i>*</i></span><input {...form.register('amount', { valueAsNumber: true })} inputMode="decimal" placeholder="2 500 000" aria-invalid={!!form.formState.errors.amount} />{form.formState.errors.amount && <small>{form.formState.errors.amount.message}</small>}</label><label className="field currency-field"><span>Měna <i>*</i></span><select {...form.register('currency')}><option>CZK</option><option>EUR</option><option>USD</option></select></label></div>
          <div className="form-info"><ShieldCheck size={20} /><span><strong>Bezpečné opakování požadavku</strong><small>Každé odeslání používá unikátní idempotency key. Duplicitní žádost při síťové chybě nevznikne.</small></span></div>
      ...
      <aside className="process-card"><p className="eyebrow">Proces zpracování</p><h2>Co bude následovat</h2><ol><li className="active"><span><Check size={15} /></span><div><strong>Založení žádosti</strong><small>Základní parametry úvěru</small></div></li><li><span>2</span><div><strong>Úvěrové posouzení</strong><small>Kontrola specialistou</small></div></li><li><span>3</span><div><strong>Rozhodnutí</strong><small>Schválení nebo zamítnutí</small></div></li></ol><div className="process-note"><Info size={18} /><span>Jde o portfolio demonstrátor bez reálných klientských dat.</span></div></aside>
    </div>
  </div>
}
